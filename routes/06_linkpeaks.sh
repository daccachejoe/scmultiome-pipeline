#!/bin/bash
# Step 06: link ATAC peaks to nearby genes, in parallel, one job per group.
# Reads stage 05's output (cell types called, peaks re-called grouped by
# cell type). Unlike subcluster/SCENIC+ (routes/downstream/), this stage
# is part of the standard trunk -- peak-gene linking is a near-universal
# step in multiome analysis, not an optional side analysis.
#
...
scripts/06a_linkpeaks_split.R "$INPUT_RDS" "$GROUPING_VAR" "$OUT_PREFIX"

GROUP_LIST_FILE="output/RDS-files/${OUT_PREFIX}-06-linkpeaks-groups.txt"
...
while IFS= read -r group; do
    [ -z "$group" ] && continue
    GROUP_RDS="output/RDS-files/${OUT_PREFIX}-06-linkpeaks-group-${group}-obj.RDS"
    OUT_LOG="scripts/outs/06-linkpeaks-${group}-%J.out"
    if [ "$SCHEDULER" == "slurm" ]; then
        jid=$(sbatch --parsable -c 6 -p "$slurm_partition" --mem-per-cpu 12000 -o "$OUT_LOG" -t 0-12:00 --export=ALL \
            --wrap="scripts/06b_linkpeaks_group.R $GROUP_RDS $group $OUT_PREFIX")
        JOB_IDS+=("$jid")
    elif [ "$SCHEDULER" == "lsf" ]; then
        jid=$(bsub -P "$lsf_project" -q "$lsf_queue" -n 6 -R "rusage[mem=12000]" -R span[hosts=1] -o "$OUT_LOG" -W 12:00 \
            "scripts/06b_linkpeaks_group.R $GROUP_RDS $group $OUT_PREFIX" | grep -oE '[0-9]+' | head -1)
        JOB_IDS+=("$jid")
    else
        echo "No job scheduler available; running group $group inline"
        scripts/06b_linkpeaks_group.R "$GROUP_RDS" "$group" "$OUT_PREFIX"
    fi
done < "$GROUP_LIST_FILE"

GROUP_CSV=$(paste -sd, "$GROUP_LIST_FILE")
MERGE_LOG="scripts/outs/06-linkpeaks-merge-%J.out"
...
if [ "$SCHEDULER" == "slurm" ] && [ ${#JOB_IDS[@]} -gt 0 ]; then
    ...
    sbatch -c 4 -p "$slurm_partition" --mem-per-cpu 8000 -o "$MERGE_LOG" -t 0-2:00 \
        ...
        --wrap="scripts/06c_linkpeaks_merge.R $OUT_PREFIX $GROUP_CSV"
    ...
    bsub -P "$lsf_project" -q "$lsf_queue" -n 4 -R "rusage[mem=8000]" -o "$MERGE_LOG" -W 2:00 -w "$DEP" \
        "scripts/06c_linkpeaks_merge.R $OUT_PREFIX $GROUP_CSV"
else
    scripts/06c_linkpeaks_merge.R "$OUT_PREFIX" "$GROUP_CSV"
fi
